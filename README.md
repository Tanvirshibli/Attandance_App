...
- **Employee services (v2.2.0):** footer **Services** tab (Attendance Report, Leave, Payments hub with web-aligned Receive payment ADD/SAVE, Sales Info with live person-sales report + Post booking (feed/chicks, chicks Zone from `all-dealer-lists` `zoneList`) / Post sale (egg, fertilizer, live bird, cull bird) with dealer dropdowns, Vehicles (fleet list → Maintenance / Trips per vehicle), **Farm & Dealer** field collection with Create / View all hub cards and the paper farm visit report (v2.2.3+60), Geo Tracking). Auth-wise receive uses live setup + dealer lists; HRM payments remain demo by default; see [docs/SALES_AND_PAYMENTS_API_CONTRACT.md](docs/SALES_AND_PAYMENTS_API_CONTRACT.md), [docs/VEHICLES_API.md](docs/VEHICLES_API.md), and [docs/FARM_DEALER_MOBILE.md](docs/FARM_DEALER_MOBILE.md)
...
- Geo: live Google Maps window (Standard / Terrain / Hybrid Satellite), full-screen map with exit, 5-min foreground timer, WorkManager, ongoing notification; FCM wake registers tokens and handles `geo_wake` data pushes when `android/app/google-services.json` is present (see `.example`)
- Persistent per-install device identity sent with attendance and face-registration requests
- Face capture: one rounded frame; success tick centered on that painted guide; step/coaching text centered above it; live fill and still size share a 16% floor; screen stays on during registration and check-in
