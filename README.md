...
- Geo: live `flutter_map` window with switchable Standard / Detailed / Satellite layers, 5-min foreground timer, WorkManager, ongoing notification; FCM wake registers tokens and handles `geo_wake` data pushes when `android/app/google-services.json` is present (see `.example`)
