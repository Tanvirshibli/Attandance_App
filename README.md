...
- **Employee services (v2.2.0):** footer **Services** tab (Attendance Report, Leave, Payments hub with web-aligned Receive payment ADD/SAVE, Sales Info with live person-sales report + Post booking (feed/chicks, chicks Zone from `all-dealer-lists` `zoneList`) / Post sale (egg, fertilizer, live bird, cull bird) with dealer dropdowns, Vehicles (fleet list → Maintenance / Trips per vehicle), **Farm & Dealer** field collection, Geo Tracking). Auth-wise receive uses live setup + dealer lists; HRM payments remain demo by default; see [docs/SALES_AND_PAYMENTS_API_CONTRACT.md](docs/SALES_AND_PAYMENTS_API_CONTRACT.md), [docs/VEHICLES_API.md](docs/VEHICLES_API.md), and [docs/FARM_DEALER_MOBILE.md](docs/FARM_DEALER_MOBILE.md)
